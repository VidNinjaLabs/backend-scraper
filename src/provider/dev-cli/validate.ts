...
import { ProviderMakerOptions, makeProxiedFetcher, makeStandardFetcher, targets } from '..';
...
export async function processOptions(sources: Array<Embed | Sourcerer>, options: CommandLineArguments) {
  ...
  if (options.fetcher === 'native') {
    ...
    proxiedFetcher = makeProxiedFetcher(fetch);
  } else {
    fetcher = makeStandardFetcher(nodeFetch);
    proxiedFetcher = makeProxiedFetcher(nodeFetch);
  }
