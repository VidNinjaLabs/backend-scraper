...
import { runTopProvidersRace } from '@/runners/parallelRunner';
...
export interface ProviderControls {
  ...
  // Run top providers in PARALLEL (much faster) - returns first successful result
...
export function makeControls(ops: ProviderControlsInput): ProviderControls {
  ...
  return {
    ...
    runAllFast(runnerOps) {
      // Run top 5 providers in parallel with 20s timeout
